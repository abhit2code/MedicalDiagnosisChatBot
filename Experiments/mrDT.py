...
my_model = Model(
    name="DecisionTreeClassifier Model created. r{}".format(random.randint(0, 100)),
    description="DecisionTreeClassifier Model created with project id: {}".format(my_project.project_id),
...
DecisionTreeClf =  DecisionTreeClassifier(criterion = 'entropy', random_state = 1)
DecisionTreeClf.fit(X_train,y_train)

y_preds = DecisionTreeClf.predict(X_test)
y_preds_probs = DecisionTreeClf.predict_proba(X_test)
